...
import type { Certificate as SocialCertificate } from '@shared/types'
...
/**
 * Same offline canvas renderer for the profile-page certificates (exam /
 * level-test / course), which use the shared social Certificate shape.
 */
export function downloadProfileCertificate(cert: SocialCertificate, learnerName: string): void {
  const W = 1400
  const H = 990
  const canvas = document.createElement('canvas')
  canvas.width = W
  ...
  const sourceLabel =
    cert.source === 'exam' ? 'EXAM RESULT' : cert.source === 'level-test' ? 'LEVEL TEST' : 'COURSE COMPLETION'
  ctx.fillStyle = '#93c5fd'
  ctx.font = '600 30px Georgia, serif'
  ctx.fillText(`SPEAKAI · CERTIFICATE · ${sourceLabel}`, center, 180)
  ...
  ctx.fillStyle = '#ffffff'
  ctx.font = 'bold 76px Georgia, serif'
  ctx.fillText(learnerName, center, 420)
  ...
  ctx.fillStyle = '#cbd5e1'
  ctx.font = '32px Georgia, serif'
  ctx.fillText('for achieving', center, 530)

  ctx.fillStyle = '#ffffff'
  ctx.font = 'bold 48px Georgia, serif'
  wrapText(ctx, cert.title, center, 600, W - 320, 56)

  ctx.fillStyle = '#34d399'
  ctx.font = 'bold 40px Georgia, serif'
  ctx.fillText(cert.detail, center, 730)

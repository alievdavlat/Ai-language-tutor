...
const EDIT_FIELDS: FieldDef[] = [
  ...
  { name: 'role', label: 'Role', type: 'select', options: [{ value: 'student', label: 'Student' }, { value: 'teacher', label: 'Teacher' }] },
  { name: 'level', label: 'Level', type: 'select', options: () => [{ value: '', label: '—' }, ...levels.list().map((l) => ({ value: l.code, label: l.name }))] },
  { name: 'targetLanguage', label: 'Learning', type: 'select', options: () => SUPPORTED_LANGUAGES.map((l) => ({ value: l.code, label: `${l.flag} ${l.name}` })) },
  { name: 'country', label: 'Country', type: 'text' },
  { name: 'bio', label: 'Bio', type: 'textarea', full: true, rows: 2 }
]
...
export default function UsersPage(): JSX.Element {
  ...
  return (
    <div className="flex flex-col gap-5">
      <div>
        <h1 className="text-xl font-bold tracking-tight text-white flex items-center gap-2"><IconUsers className="w-5 h-5 text-slate-400" /> Users</h1>
        <p className="text-sm text-slate-500 mt-0.5">Real member directory — search, change roles, ban, and inspect activity.</p>
      </div>

      <div className="flex flex-wrap items-center gap-3">
        <div className="relative flex-1 max-w-sm">
          <IconSearch className="w-4 h-4 absolute left-3 top-1/2 -translate-y-1/2 text-slate-500" />
          <input value={q} onChange={(e) => setQ(e.target.value)} placeholder="Search name or email…" className="w-full rounded-lg bg-white/[0.04] border border-white/10 pl-9 pr-3 py-2 text-sm text-white placeholder:text-slate-500 focus:border-brand-400 focus:outline-none" />
        </div>
        ...
        <span className="text-xs text-slate-500 tabular-nums">{filtered.length} of {all.length}</span>
      </div>
      ...
      <DataTable
        ...
        columns={[
          { key: 'name', label: 'Member', render: (u) => (
          ...
          { key: 'role', label: 'Role', cls: 'w-24', render: (u) => (
          ...
          { key: 'joined', label: 'Joined', cls: 'w-28', render: (u) => <span className="text-slate-400">{new Date(u.createdAt).toLocaleDateString()}</span> },
          { key: 'status', label: 'Status', cls: 'w-28', render: (u) => {
            const s = bannedMap.get(u.id)
            if (s?.banned) return <span className="inline-flex rounded-md text-[10px] font-bold uppercase tracking-wider px-1.5 py-0.5 bg-rose-500/15 text-rose-200">Banned</span>
            if (s && s.warnings > 0) return <span className="inline-flex rounded-md text-[10px] font-bold uppercase tracking-wider px-1.5 py-0.5 bg-amber-500/15 text-amber-200">{s.warnings} warning{s.warnings === 1 ? '' : 's'}</span>
            return <span className="text-emerald-300/80 text-[11px] font-semibold">Active</span>
          } }
        ]}
        actions={(u) => {
      ...
      {open && (
        <Drawer
          ...
          footer={
            <>
              <button onClick={() => void warn(open)} className="rounded-lg px-3 py-2 text-sm font-semibold text-amber-200 hover:bg-amber-500/10">Warn</button>
              <button onClick={() => void toggleBan(open)} className="rounded-lg px-3 py-2 text-sm font-semibold text-rose-200 hover:bg-rose-500/10">
                {bannedMap.get(open.id)?.banned ? 'Unban user' : 'Ban user'}
              </button>
              <div className="flex-1" />
              <button onClick={() => setOpen(null)} className="rounded-lg px-4 py-2 text-sm font-semibold text-slate-300 hover:bg-white/[0.06]">Cancel</button>
              <button onClick={() => void saveUser()} disabled={saving} className="rounded-lg px-5 py-2 text-sm font-bold bg-brand-500 text-white hover:bg-brand-400 disabled:opacity-50">{saving ? 'Saving…' : 'Save'}</button>
            </>
          }
        >
          {/* Stats strip */}
          {stats && (
            <div className="grid grid-cols-4 gap-2 mb-5">
              {[
                ...
                { v: stats.lessonsCompleted, l: 'Lessons' },
                { v: stats.wordsLearned, l: 'Words' }
              ].map((s) => (
          ...
          {/* Recent activity */}
          <div className="mt-6">
            <h3 className="text-[11px] uppercase tracking-widest text-slate-500 font-bold mb-2">Recent activity</h3>
            {activity.length === 0 ? (

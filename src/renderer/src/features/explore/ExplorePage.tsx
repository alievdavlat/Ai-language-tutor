      ...
      <div className="px-6 py-6 w-full flex flex-col gap-5">
        ...
        <div className="rounded-card bg-gradient-to-br from-brand-500/20 via-violet-500/15 to-pink-500/15 border border-white/10 p-6">
          ...
          <p className="text-sm text-slate-300 mt-1">
            Top courses, trending creators, live streams and study buddies — all learning {lang.name}.
          </p>
          <div className="relative mt-4">
            ...
            <input
              ...
              placeholder={`Search ${lang.name} courses, people, videos, posts…`}
          ...
          <div className="flex items-center gap-2 mt-3 flex-wrap">
            {['IELTS', 'Pronunciation', 'Grammar', 'Business', 'Travel', 'Conversation'].map((t) => (
            ...
            {q && (
              <button onClick={() => setQ('')} className="rounded-full bg-white/[0.06] hover:bg-white/[0.12] text-slate-400 text-xs font-semibold px-3 py-1.5">Clear ✕</button>
            )}
          </div>
        </div>
        ...
        {tab === 'videos' && (
          videoTiles.length === 0 ? (
            <p className="text-sm text-slate-400 text-center py-8">No videos match your search.</p>
          ) : (
        ...
        {tab === 'people' && (
          matchedPeople.length === 0 ? (
            <p className="text-sm text-slate-400 text-center py-8">No people match your search.</p>
          ) : (
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3">
              {matchedPeople.map((u) => (

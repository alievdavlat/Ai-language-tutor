        ...
        <div className="rounded-card p-5 bg-gradient-to-br from-amber-500/15 to-rose-500/15 border border-amber-400/20 flex items-center gap-4">
          ...
          <div className="flex-1">
            <p className="text-sm font-bold text-white">
              {freeLeft > 0 ? `You can submit ${freeLeft} free item${freeLeft === 1 ? '' : 's'} today` : `Submissions cost ${SUBMIT_COST} karma today`}
            </p>
            <p className="text-xs text-amber-100/80">
              Each review you give earns karma · earned {karma?.earnedTotal ?? 0} all-time.
            </p>
          </div>
  ...
  if (items.length === 0) {
    return (
      <div className="rounded-card border border-white/10 bg-white/[0.025] p-8 text-center">
        ...
        <p className="text-sm font-semibold text-white">Nothing to review right now</p>
        <p className="text-xs text-slate-500 mt-1">Check back soon — or submit your own work to get reviewed.</p>
      </div>
    )
  }
          ...
          <article key={s.id} className="rounded-card border border-white/10 bg-white/[0.025] p-4">
            <header className="flex items-center gap-3 mb-3">
              ...
              <div className="flex-1 min-w-0">
                <p className="text-sm font-bold text-white">
                  {author?.name ?? 'A learner'} <span className="text-xs text-slate-500">· {s.level}</span>
                </p>
                <p className="text-[11px] text-slate-500">{timeAgo(s.createdAt)} · {s.reviewCount} review{s.reviewCount === 1 ? '' : 's'}</p>
              </div>
              <span
                className={cn(
            ...
            {reviewing === s.id ? (
              ...
              <div className="flex items-center justify-between mt-3">
                ...
                <button onClick={() => setReviewing(s.id)} className="btn-primary text-xs px-4 py-1.5">Review</button>
              </div>
            )}
          </article>
        )
  ...
  return (
    <div className="mt-3 rounded-xl border border-brand-400/30 bg-brand-500/5 p-3 flex flex-col gap-3">
      <div className="flex items-center gap-1">
        <span className="text-[11px] uppercase tracking-widest text-slate-500 font-semibold mr-2">Rating</span>
        {[1, 2, 3, 4, 5].map((n) => (
          <button key={n} onClick={() => setRating(n)} title={`${n} stars`} className="p-0.5">
      ...
      <textarea
        ...
        placeholder="Be specific and kind — what worked, and one thing to improve…"
      />
      <div className="flex items-center justify-end gap-2">
        ...
        <button onClick={() => void submit()} disabled={text.trim().length < 10 || saving} className="btn-primary text-xs px-4 py-1.5">
          {saving ? 'Posting…' : `Post review · +${submission.reward} karma`}
        </button>
      </div>
    </div>
  )
...
function ReceivedTab({
  ...
  return (
    <div className="flex flex-col gap-5">
      {/* Reviews received */}
      <div className="flex flex-col gap-3">
        <p className="text-xs uppercase tracking-widest text-slate-500 font-bold">Feedback received</p>
        {received.length === 0 ? (
          <p className="text-xs text-slate-500">No reviews yet — they'll appear here as people review your work.</p>
        ) : (
                ...
                <div className="flex items-center gap-2.5">
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-xs text-slate-300">
                      <b className="text-white">{reviewer?.name ?? 'A peer'}</b>
                  ...
                  >
                    <IconHeart className="w-3.5 h-3.5" /> {review.thanked ? 'Thanked' : 'Thank'}
                  </button>
                </div>
              </article>
            )
      ...
      {/* My submissions */}
      <div className="flex flex-col gap-3">
        <p className="text-xs uppercase tracking-widest text-slate-500 font-bold">Your submissions</p>
        {mine.length === 0 ? (
          <p className="text-xs text-slate-500">You haven't submitted anything yet.</p>
        ) : (
          mine.map((s) => (
            <article key={s.id} className="rounded-card border border-white/10 bg-white/[0.025] p-4 flex items-center gap-3">
              ...
              >
                {s.reviewCount > 0 ? `${s.reviewCount} review${s.reviewCount === 1 ? '' : 's'}` : 'Awaiting review'}
              </span>
            </article>
          ))
        )}
  ...
  return (
    <div className="rounded-card border border-white/10 bg-white/[0.025] p-5 flex flex-col gap-4">
          ...
          {(['writing', 'speaking'] as const).map((k) => (
            ...
            >
              <p className="inline-flex items-center gap-2 text-sm font-bold text-white">
                ...
                {k === 'writing' ? 'Writing' : 'Speaking'}
              </p>
              <p className="text-[11px] text-slate-400 mt-0.5">{k === 'writing' ? 'Essay, email, message' : 'Paste your transcript'}</p>
            </button>
          ))}
        </div>
      </div>
      ...
      <div>
        ...
        <input value={topic} onChange={(e) => setTopic(e.target.value)} className="input mt-1.5" placeholder="e.g. IELTS Task 2 — climate change" />
      </div>

      <div>
        <label className="text-xs uppercase tracking-widest text-slate-500 font-semibold">
          {kind === 'writing' ? 'Your writing' : 'Your transcript'}
        </label>
        <textarea
          value={content}
          onChange={(e) => setContent(e.target.value)}
          className="input mt-1.5 min-h-[120px] resize-none"
          placeholder={kind === 'writing' ? 'Paste or type your work here…' : 'Type what you said (or paste an auto-transcript)…'}
        />
        <p className="text-[10px] text-slate-500 mt-1">{words} words</p>
      </div>
      ...
      <div className="flex items-center justify-between">
        <span className="text-xs text-slate-400">
          {freeLeft > 0 ? `Free · ${freeLeft} left today` : `Cost: ${SUBMIT_COST} karma (balance ${balance})`}
        </span>
